...
```{r}
rand_data <- read.csv("RandomCompnay.csv")
# Simulate the posterior
model_1 <- stan_glm(
  Earnings_next_year ~ EARNINGS, data = rand_data,
  family = gaussian,
  prior_PD = FALSE,
...
model_2_prior_with_hiar <- stan_glmer(
  Earnings_next_year ~ EARNINGS + Earnings_1_years_ago + Earnings_2_years_ago + SALES + SALES_1_YEAR_AGO + SALES_2_YEAR_AGO + Sector + (1 | COMPANY), data = rand_data_update, 
  family = gaussian,
...
```{r}
pred <- posterior_predict(model_2, newdata = rand_data_update %>% filter(YEAR == 2020))
```
