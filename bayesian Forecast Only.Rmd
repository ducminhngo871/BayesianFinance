...
myts_KO <- ts(vector_KO, start=c(1999), end=c(2015), frequency=1)
...
# unique_COMPANY <-unique_COMPANY[unique_COMPANY != c("ABBV", "ALL", "AMCR", "AMGN", "ANTM", "APA", "AVGO", "AXP", "BA", "BAC", "BIIB", "CARR", "COP","CSCO", "CTVA", "CVX", "DOW", ]
...
```{r}
one_year_only <- data_elimatedO %>% 
  group_by(COMPANY) %>% 
  mutate(number_of_year = sum(YEAR)) %>% 
  filter(number_of_year < 2021)

COMPANY_one_year <- one_year_only$COMPANY
unique_COMPANY_one_year <- unique(COMPANY_one_year)

  #filter(number_of_year < 2)
  # filter(COMPANY == "ABBV") %>% 
  # count(YEAR)

unique_COMPANY_one_year
```

```{r}
## 61, 62, 63, 64, 90, DALcó thể bị lỗi
data_elimatedO %>% 
  filter(COMPANY == unique_COMPANY[152])
```
