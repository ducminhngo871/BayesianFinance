...
test_1 <- testing %>% 
  ...
  select(c("Sector","YEAR","Earnings_next_year_Scaled","EARNINGS_Scaled","EARNINGS_1_YEAR_AGO"))
predict_next_year <- posterior_predict(
  Model, 
  newdata = data.frame(test_1))
predicted_median <- median(predict_next_year)

most_recent_year <- test_1$YEAR + 1
actual <- data %>% 
